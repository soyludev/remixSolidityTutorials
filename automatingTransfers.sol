...
contract myGame {
    ...
    function addPlayer(string memory firstName, string memory lastName) private {   //memory = how it saved
        Player memory newPlayer = Player(msg.sender, Level.Novice,firstName, lastName, block.timestamp);
            
        players[msg.sender] = newPlayer;//players.push(Player(firstName, lastName));
    ...
    function joinGame(string memory firstName, string memory lastName) payable public {
        require(msg.value == 25 ether, "The joining fee is 25 ethers");
    ...
    function payOutWinners(address loserAddress) payable public{
        ...
        uint payoutVal = msg.value/(playerCount-1);
        for (uint i=0; i < playersInGame.length; i++){
            ...
            if(currentPlayerAddress != loserAddress){
                payable(currentPlayerAddress).transfer(payoutVal);
            }
